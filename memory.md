...
- Besu 區塊 URL：架最輕量 explorer（Expedition／Ethereum Lite Explorer，指向 8545），組真實 `/tx/<hash>`。
...
- 進度：Gemini 關鍵幀測試 3/3 通過；`rerun_p4.js` 已寫、dry-run 單支通過（DBH→碳、關鍵幀、樹種 fallback 皆正常）；尚未跑批次、未做 dashboard02、未架 explorer。
